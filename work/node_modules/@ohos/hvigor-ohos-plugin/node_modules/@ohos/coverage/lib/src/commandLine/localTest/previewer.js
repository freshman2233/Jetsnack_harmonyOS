"use strict";
Object.defineProperty(exports, "__esModule", { value: true });
exports.executeCoverageTest = void 0;
const path = require("path");
const fs = require("fs");
const node_child_process_1 = require("node:child_process");
const coverageParser_1 = require("./coverageParser");
const portutil_1 = require("./portutil");
const standardErrorMessage_1 = require("../../error/standardErrorMessage");
const abcDirRelative = '.test/default/intermediates/assets/default/ets';
const resoucrceDirRelative = '.test/default/intermediates/res/default';
const logPathRelative = '.test/default/intermediates/test/coverage_data/coverage.log';
const phonePathRelative = '.test/testability/phoneSettingConfig_Phone.json';
const loaderPathRelative = '/.test/default/intermediates/loader/default/loader.json';
const getSpawnChild = (previewerFullDir, abcDir, phoneConfigPath, resourceDirectory, loaderPath, portNum, hmsSdkApiPath, runtimeType) => {
    let previewerPath = previewerFullDir;
    if (process.platform === 'win32') {
        previewerPath = path.join(previewerFullDir, 'Previewer.exe');
    }
    else if (process.platform === 'darwin' || process.platform === 'linux') {
        previewerPath = path.join(previewerFullDir, 'Previewer');
    }
    if (hmsSdkApiPath === undefined) {
        hmsSdkApiPath = '';
    }
    if (runtimeType === undefined) {
        runtimeType = '';
    }
    if (runtimeType === 'HarmonyOS') {
        const child = (0, node_child_process_1.spawn)(previewerPath, [
            '-d', '',
            '-abp', '.test/testability/TestAbility.abc',
            '-ilt', 'true',
            '-refresh', 'region',
            '-projectID', '743055776',
            '-ts', 'trace_7947_commandPipe',
            '-j', abcDir,
            '-s', 'phone_1702972840143_1',
            '-cpm', 'false',
            '-device', 'phone',
            '-shape', 'rect',
            '-sd', '480',
            '-or', '1080', '2340',
            '-cr', '1080', '2340',
            '-f', phoneConfigPath,
            '-n', 'entry',
            '-av', 'ACE_2_0',
            '-url', '../../../.test/testability/pages/Index',
            '-pages', 'main_pages',
            '-arp', resourceDirectory,
            '-pm', 'Stage',
            '-l', 'zh_CN',
            '-cm', 'light',
            '-o', 'portrait',
            '-ljPath', loaderPath,
            '-lws', portNum,
            '-hsp', hmsSdkApiPath,
        ], { windowsHide: true, cwd: previewerFullDir });
        return child;
    }
    else {
        const child = (0, node_child_process_1.spawn)(previewerPath, [
            '-d', '',
            '-abp', '.test/testability/TestAbility.abc',
            '-ilt', 'true',
            '-refresh', 'region',
            '-projectID', '743055776',
            '-ts', 'trace_7947_commandPipe',
            '-j', abcDir,
            '-s', 'phone_1702972840143_1',
            '-cpm', 'false',
            '-device', 'default',
            '-shape', 'rect',
            '-sd', '480',
            '-or', '1080', '2340',
            '-cr', '1080', '2340',
            '-f', phoneConfigPath,
            '-n', 'entry',
            '-av', 'ACE_2_0',
            '-url', '../../../.test/testability/pages/Index',
            '-pages', 'main_pages',
            '-arp', resourceDirectory,
            '-pm', 'Stage',
            '-l', 'zh_CN',
            '-cm', 'light',
            '-o', 'portrait',
            '-ljPath', loaderPath,
            '-lws', portNum,
        ], { windowsHide: true, cwd: previewerFullDir });
        return child;
    }
};
const portSet = new Set();
const executeCoverageTest = async (testPreviewerParam) => {
    const sdkApiPath = testPreviewerParam.sdkApiPath;
    const modulePath = testPreviewerParam.modulePath;
    const Logger = testPreviewerParam.logger;
    const hspDependencies = testPreviewerParam.hspDependencies;
    let previewerFullDir = path.join(sdkApiPath, '/common/bin');
    const abcDir = path.join(modulePath, abcDirRelative);
    const resourceDir = path.join(modulePath, resoucrceDirRelative);
    const logPath = path.join(modulePath, logPathRelative);
    const phoneConfigPath = path.join(modulePath, phonePathRelative);
    const loaderPath = path.join(modulePath, loaderPathRelative);
    const hmsSdkApiPath = testPreviewerParam.hmsSdkApiPath;
    const runtimeType = testPreviewerParam.runtimeType;
    if (fs.existsSync(logPath)) {
        fs.unlinkSync(logPath);
    }
    if (!fs.existsSync(logPath)) {
        fs.mkdirSync(path.dirname(logPath), { recursive: true });
    }
    const portNum = (0, portutil_1.findPort)(40000, 0, Logger, portSet);
    if (portNum < 0) {
        Logger.error('No port available');
        return new Promise((resolve) => {
            resolve();
        });
    }
    let previewPromise = new Promise((resolve, reject) => {
        const child = getSpawnChild(previewerFullDir, abcDir, phoneConfigPath, resourceDir, loaderPath, `${portNum}`, hmsSdkApiPath, runtimeType);
        child.on('close', () => {
            portSet.delete(portNum);
        });
        child.stdout.on('data', async (data) => {
            try {
                fs.writeFileSync(logPath, data, { flag: 'a+' });
                // file written successfully
            }
            catch (err) {
                reject(err);
            }
            if (!isShouldKillPreviewer(data, Logger)) {
                return;
            }
            child.kill();
            await (0, coverageParser_1.parseCoverageData)({
                modulePath: modulePath,
                projectPath: testPreviewerParam.projectPath,
                hdcPath: testPreviewerParam.sdkApiPath,
                coverage: testPreviewerParam.coverage,
                logger: Logger,
                isLocalTest: true,
                dependenciesPaths: hspDependencies,
                bundleName: '',
                bundleType: ''
            })
                .then(() => {
                resolve();
            })
                .catch(reject);
        });
    });
    return previewPromise;
};
exports.executeCoverageTest = executeCoverageTest;
function isShouldKillPreviewer(data, Logger) {
    if (data.indexOf(coverageParser_1.ENGINE_LOG_ERROR_MESSAGE_LOG) > -1) {
        return true;
    }
    const dataStr = data.toString();
    if (/Throw error:.*does not provide an export name.*which imported by.*/.test(dataStr)) {
        const errMsg = (standardErrorMessage_1.getFormattedErrorMsg)('PRIVIEWER_LOG_ERROR').replace('{msg}', formatErrorMessage(data));
        Logger.error(errMsg);
        return true;
    }
    if (/ArkCompiler: [\[ArkRuntime Log\]]*?[a-zA-Z]+?Error:/.test(dataStr)) {
        const errMsg = (standardErrorMessage_1.getFormattedErrorMsg)('ARK_COMPILER_ERROR').replace('{msg}', formatErrorMessage(data));
        Logger.error(errMsg);
        return true;
    }
    if (data.indexOf('The AbilityDelegator.finishTest') > -1) {
        return true;
    }
    // 测试任务执行结束的标志
    if (data.indexOf('OHOS_REPORT_STATUS: taskconsuming') > -1) {
        return true;
    }
    return false;
}
function formatErrorMessage(content) {
    const THROW_ERR = 'Throw error:';
    const ERR_MSG = 'Error message:';
    let dataStr = String(content);
    const lines = dataStr.split('\n');
    for (const line of lines) {
        let errIndex = line.indexOf(ERR_MSG);
        if (errIndex === -1) {
            errIndex = line.indexOf(THROW_ERR);
        }
        if (errIndex > -1) {
            let formatted = line.substring(errIndex).replace(THROW_ERR, ERR_MSG);
            if (formatted.includes(ERR_MSG)) {
                formatted = formatted.substring(ERR_MSG.length);
            }
            return formatted.trim();
        }
    }
    return dataStr.trim();
}
