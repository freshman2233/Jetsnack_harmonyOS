...
class OhLocalComponentLoader extends abstract_local_component_loader_1.AbstractLocalComponentLoader {
    ...
    convertComponent(jsonObject, uniPackage) {
        ...
        const fullApiVersion = jsonObject.fullApiVersion;
        if (!(0, utils_1.isEmpty)(fullApiVersion)) {
            componentDto.fullApiVersion = new abstract_component_dto_1.ApiVersion(fullApiVersion);
        }
        else {
            componentDto.fullApiVersion = new abstract_component_dto_1.ApiVersion(jsonObject.apiVersion);
        }
        if (component_contants_1.ComponentPath.TOOLCHAINS === componentDto.getPath() &&
            ...
            componentDto.fullApiVersion = new abstract_component_dto_1.ApiVersion(oh_component_constants_1.TOOLCHAINS_3_0_0_0_API7_MAPPING.toString()); // 需要迁移的toolchains的apiversion固定设置为7
        }
    ...
    _isSameMetaVersion(dto) {
        ...
        const compareResult = (0, utils_1.isInMetaVersionList)(metaVersion, oh_component_constants_1.SUPPORT_META_VERSION_LIST);
        if (compareResult !== 0) {
            this._inCompatibleComponents.push(dto);
            return false;
        }
