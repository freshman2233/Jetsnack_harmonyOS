/*
  @license
	Rollup.js v3.19.1-htrunk1.AllScenario.devecostudio.r35
	Fri, 10 Apr 2026 09:23:40 GMT - commit 5d10cd82a819f28a4089c75cacb6ba55ffe97cdb
...
import { picomatchExports, getAugmentedNamespace, fseventsImporter, createFilter, rollupInternal, getLastReUsedFiles, clearLastReUsedFiles } from './rollup.js';
...
class Watcher {
    ...
    invalidate(file) {
        ...
        this.buildTimeout = setTimeout(async () => {
            this.buildTimeout = null;
            try {
                this.lastChangedFiles = new Map(this.invalidatedIds);
...
class Task {
    ...
    async hotCompile() {
        ...
        try {
            ...
            await result.triggerBundleEnd(true);
            await this.watcher.emitter.emit('event', {
                code: 'BUNDLE_END',
                duration: Date.now() - start,
                hotCompile: true,
                input: this.options.input,
                lastReUsedFiles: getLastReUsedFiles(),
                output: this.outputFiles,
                result
            });
            clearLastReUsedFiles();
        }
        catch (error) {
            if (!this.closed) {
    ...
    async run() {
        ...
        await this.watcher.emitter.emit('event', {
            code: 'BUNDLE_START',
            input: this.options.input,
            lastChangedFiles: this.watcher.lastChangedFiles,
            output: this.outputFiles
        });
        let result = null;
        try {
            if (this.options.stopHotCompile) {
                await this.watcher.emitter.emit('event', {
            ...
            if (this.options.hotReloadBuild) {
                this.options.stopHotCompile = true;
            }
            this.updateWatchedFiles(result);
            this.skipWrite || (await Promise.all(this.outputs.map(output => result.write(output))));
            await result.triggerBundleEnd(true);
