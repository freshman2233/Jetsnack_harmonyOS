import { ModuleInfo, PluginContext } from '@ohos/hvigor-arkts-base';
export declare const importee2filePath: Map<string, string>;
export interface RollupNodeResolveOptions {
    ...
    rootDir?: string;
    /**
     * If `true`, the plugin won't repeatedly call realpath with same path in one build process.
     * @default false
     */
    useRealpathCache?: boolean;
    /**
     * packageName -> packageInformation, used to add extra paths into import specifier list
     */
    pkgNameToPkgInfo?: Record<string, any>;
    caseSensitiveCheck?: boolean;
    sdkModulesPath?: string[];
    /**
     * 是否启用 native-resolver
     */
    useNativeResolver?: boolean;
}
/**
 * 检查import模块是否external，卡片中不能引入hsp模块
 * @param harExternal har模块Map<hsrExp, hsrName>集合
 * @param hspExternal hsp模块Map<hspExp, hspName>集合
 * @param importee from后面的值
 * @param importer 引用的文件
 * @param projectConfig projectConfig
 */
export declare function isExternal(harExternal: Map<RegExp, string>, hspExternal: Map<RegExp, string>, importee: string, importer: string | undefined, projectConfig?: any): boolean;
export declare function isBackslashExternal(harWithBackSlashExternal: Map<RegExp, string>, hspWithBackslashExternal: Map<RegExp, string>, importee: string): boolean;
export declare let idToPackageInfo: Map<string, any>;
export declare function nodeResolve(opts?: RollupNodeResolveOptions): {
    name: string;
    buildStart(this: any, buildOptions: any): void;
    generateBundle: typeof clearCache;
    beforeBuildEnd(): void;
    resolveId: {
        order: string;
        handler(this: any, importee: string, importer: string | undefined, resolveOptions: any): Promise<any>;
    };
    load: typeof load;
    transform: typeof transform;
    moduleParsed(this: PluginContext, moduleInfo: ModuleInfo): void;
    getPackageInfoForId(id: string): any;
    cleanUp: () => void;
};
export declare function getExternal(harExternal: Map<RegExp, string>, hspExternal: Map<RegExp, string>, harWithBackSlashExternal: Map<RegExp, string>, hspWithBackslashExternal: Map<RegExp, string>, projectConfig: any): void;
declare function clearCache(): void;
declare function transform(this: PluginContext, code: string, id: string): void;
declare function load(this: PluginContext, importee: string): "" | "export default {};" | null;
/**
 * 处理har包去重
 */
export declare function processHarDeduplicate(pluginContext: PluginContext, resolvedId: string, canMakeExternal: boolean): {
    id: string;
    external: boolean;
    meta: object;
} | null;
